...
//private:
    ...
    void ComputeWD();
    void ComputeWD2();
    double Density(const int , const double );
    double radius(const int );
    void ComputeDPhi();
    void ComputeLocalChemicalPotential();
    double DPhi(const int , const double);
...
:_L(L),_h(_L/double(M+1)),_dk(M_PI/_L)
{
    ...
    GaussW=new double [N_gauss];
    GaussX=new double [N_gauss];

    gauleg(N_gauss,GaussX,GaussW);
}
...
void FMT<N,M>::ComputeWD()
{
    ...
    for(int i=0;i<N;i++)
    {
        for(int j=0;j<M;j++)
        {
            ...
            xmin=fabs(_r[j]-_a[i]*0.5);
            xmin=xmin<(_a[FixHS]+_a[i])*0.5?(_a[FixHS]+_a[i])*0.5:xmin;
            xmax=_r[j]+_a[i]*0.5;
            if(xmax<xmin)
            {
                _wd(i,2,j)=0;
            ...
            else
            {
                ...
                {
                    gx=xmean-xlength*GaussX[k];
                    gw=GaussW[k]*xlength;
                    _wd(i,2,j)+=gx*Density(i,gx)*gw;
                    _wd(i,3,j)+=gx*Density(i,gx)*(_a[i]*_a[i]/4.-(_r[j]-gx)*(_r[j]-gx))*gw;
                    _wd(i,5,j)+=gx*Density(i,gx)*(_r[j]*_r[j]+_a[i]*_a[i]/4.-gx*gx)*gw;
                }
    ...
    for(int i=0;i<N;i++)
    {
        for(int j=0;j<M;j++)
        {
            ...
            for(int k=0;k<N_gauss;k++)
            {
                gx=xmean+xlength*GaussX[k];
                gw=GaussW[k]*xlength;
                _lcp(i,0,j)+=gx*DPhi(0,gx)*gw;
                _lcp(i,1,j)+=gx*DPhi(1,gx)*gw;
                _lcp(i,2,j)+=gx*DPhi(2,gx)*gw;
                _lcp(i,3,j)+=gx*DPhi(3,gx)*(_a[i]*_a[i]/4.-(_r[j]-gx)*(_r[j]-gx))*gw;
                _lcp(i,4,j)+=DPhi(4,gx)*(-_r[j]*_r[j]+_a[i]*_a[i]/4.+gx*gx)*gw;
                _lcp(i,5,j)+=DPhi(5,gx)*(-_r[j]*_r[j]+_a[i]*_a[i]/4.+gx*gx)*gw;
            }
...
template<int N, int M>
double FMT<N,M>::Density(const int i , const double r)
{
    if(r<0)
    {
        return Density(i,fabs(r));
    }
    else if(r<=_h)
    {
        return (_density(i,0)*(2.*_h-r)+_density(i,1)*(r-_h))/_h;
    }
    else if (r>=_r[M-1])
    {
    ...
    {
        int j=floor(r/_h)-1.;
        return (_density(i,j)*(_r[j+1]-r)+_density(i,j+1)*(r-_r[j]) )/_h;
    }
//return fabs(r); //for test algorithm
//if(r<0)
//{
//    return Density(i,fabs(r));
//}
...
template<int N, int M>
double FMT<N,M>::DPhi(const int i , const double r)
{
    if(r<0)
    {
        if(i<=3)
        {
            return DPhi(i,fabs(r));
        }
        else
        {
            return -DPhi(i,fabs(r));
    ...
    {
        return (_dPhi(i,0)*(2.*_h-r)+_dPhi(i,1)*(r-_h))/_h;
    }
    else if (r>=_r[M-1])
    {
        return _dPhi(i,M-1);
    ...
    {
        int j=floor(r/_h)-1;
        return (_dPhi(i,j)*(_r[j+1]-r)+_dPhi(i,j+1)*(r-_r[j]) )/_h;
    }
}
